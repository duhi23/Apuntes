...
\vspace*{\baselineskip}
\vspace*{\baselineskip}
{\Huge\scshape Apuntes Máster}\\[10mm]
%{\Large \includegraphics[scale=0.25]{uah.pdf}}\\[35mm]
{\Huge\scshape Seguros  \\[5mm]
de no Vida} \\ [\baselineskip]
{\Large\bfseries Primera Edición}\\[0.3 \textheight]
{\Large Deskito Polanco.}\\
%{\large\scshape MS-PLUS, Inc.}\par
\vfill
\begin{center}
{\scshape Quito-Ecuador}\\
\rule{\textwidth}{0.5pt}
\end{center}
\vspace*{\baselineskip}
\endgroup}
...
SOme importante classes of loss distributions are the following:
\item $X~Exponential(\alpha)$ means that $X$ has density $f_X(x)=\alphae\{.\alphax}$

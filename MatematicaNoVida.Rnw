...
<<examplePlot, fig=TRUE,echo=FALSE>>=
x <- seq(0,8,0.01)
y <- exp(-0.5*x)
y1 <- exp(-1.5*x)
y2 <- exp(-1*x)

plot(x,y,type="l",main=expression(paste("Exponential distribution with parameter ",alpha)))
...
\begin{align*}
log P[\frac{M_n-b_n}{a_n}\leq x]&=log F^n(a_nx+b_n)\\
&=nlogF(a_nx+b_n)\\
&\sim\{1-F(a_nx+b_n)\}
\end{align*}
...
Thus $n^{1/\alpha}M_n\xrightarrow{d}Fr{echet(\alpha)$ as  can be expressed in terms of $F$ as follows.
...
$$\lim_{x\rightarrow\infty}\frac{log(tx)}{log x}=\lim_{x\rightarrow\infty}\frac{log t}{log x}+\frac{logx}{log x}=1\ \ \log \in R_0$$


$$log^{(0)}x=x, log^{(1)}=log x$$
$log^{(k)}=log log^{(k-1)}x$ for $k=1,2,...$

...
<<echo=FALSE, fig.align='center', fig.height=4.5, fig.width=5>>=
x <- seq(0,8,0.01)
y <- exp(-0.5*x)
y1 <- exp(-1.5*x)
y2 <- exp(-1*x)

plot(x,y,type="l",main=expression(paste("Pareto distribution with parameter ",alpha)), xlab='X', ylab='F(X)')
...
<<echo=FALSE, fig.align='center', fig.height=4.5, fig.width=5>>=
plot(seq(0,1,0.01),pweibull(seq(0,1,0.01),scale=10,shape=1,log=FALSE),
     type = "n",main="Weibull Distribution",xlab="X",ylab="F(X)")
lines(seq(0,1,0.01),pweibull(seq(0,1,0.01),scale=10,shape=1,log=FALSE),col="red")
lines(seq(0,1,0.01),pweibull(seq(0,1,0.01),scale=100,shape=0.5,log=FALSE),col="blue")
lines(seq(0,1,0.01),pweibull(seq(0,1,0.01),scale=4.7,shape=1.5,log=FALSE),col="black")
grid()
@
...
<<plot1,echo=FALSE,eval=TRUE, fig.align='center', fig.height=4.5, fig.width=5>>=
x<-c(0:10)
plot(x,log(x),type="l",ylim = c(-5,4))
lines(x,x-1)
legend(x = 2,y=2.8,legend = "x-1")
legend(x = 5,y=1.5,legend = "log x")
grid()
@
...
<<33,echo=FALSE,eval=TRUE, fig.align='center', fig.height=4.5, fig.width=5>>=
x<-c(0:5)
plot(x,x,type="n")
lines(x,4,type = "b")
grid()
...
<<06/4,echo=FALSE,eval=TRUE, fig.align='center', fig.height=4.5, fig.width=5>>=

d <- density(rnorm(1000,mean=c(25,15), sd = c(2,4)))
dens<-d
plot(dens,xaxt="n",main="",yaxt="n",xlab = "",ylab="",xlim=c(5,30))
axis(side = 1,labels = c("January","s","t","s+r","t+r","December"),at=c(5,11,15,22,26,30))
q1     <- 11
q15    <- 15
qn08   <- 22
qn02   <- 26
...
$$
 ...
 Some possible generalization of the basic risk procecss (of Lundberg). A Wiener Process is a stochastic process $\{W_t\}_{t\geq 0}$ with $W_0=0$ a.s, with continuous sample paths a.s, with independent increments and with $W_t-W_s\sim N(0,t-s)\ \ \ \forall0\leq s < t < \infty$
